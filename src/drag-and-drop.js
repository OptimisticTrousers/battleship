...
function shipDrag(shipName, playerBoard) {
        ...
        for (let i = 0; i < 10; i += 1) {
            playerBoard.gameBoard[i].forEach((e) => {
                if (e.offLimits || e.isShip)
                    document
                        .querySelector(
                            `.player-board > .cell[column='${e.column}'][row='${e.row}']`
                        )
    ...
    ship.addEventListener('dragend', () => {
        ...
        if (shipName === '.ship-1') {
            if (playerBoard.placeShip(pos2, pos1, dir, createShip(1)) === false)
                return
        }
        if (shipName === '.ship-2') {
            if (playerBoard.placeShip(pos2, pos1, dir, createShip(2)) === false)
                return
        }
        if (shipName === '.ship-3') {
            if (playerBoard.placeShip(pos2, pos1, dir, createShip(3)) === false)
                return
        }
        if (shipName === '.ship-4') {
            if (playerBoard.placeShip(pos2, pos1, dir, createShip(4)) === false)
                return
        }
